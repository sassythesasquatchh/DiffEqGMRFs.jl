...
pub fn assemble_darcy_diff_matrix(
    ...
    let mut keep = if inflated_boundary {
        Some(Vec::new())
    } else {
        None
    };
...
fn node_spacings_1d(nodes: &[f64]) -> Vec<f64> {
    nodes.windows(2).map(|w| w[1] - w[0]).collect()
}

fn apply_constraints(
    matrix: &mut Array2<f64>,
    mut rhs: Option<&mut Array1<f64>>,
...
mod tests {
    ...
    use diffeq_gmrfs_core::mesh::{
        periodic_unit_interval_discretization, uniform_unit_square_discretization,
    };
    use ndarray::array;
    ...
    fn burgers_mass_and_diffusion_match_linear_elements() {
        ...
        let k = 1.0 / h;
        let expected_diff = array![[k, -k, 0.0], [-k, 2.0 * k, -k], [0.0, -k, k],];
        assert!(g
            .iter()
    ...
    fn darcy_diffusion_assembles_interior() {
        ...
        let coeff = array![[1.0, 1.0, 1.0], [1.0, 2.0, 1.0], [1.0, 1.0, 1.0],];
