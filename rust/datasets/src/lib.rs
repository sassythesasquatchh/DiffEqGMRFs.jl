...
impl BurgersDataset {
    pub fn from_mat_file<P: AsRef<Path>>(path: P) -> Result<Self, DatasetError> {
        ...
        let nu = *visc
            .first()
            .ok_or_else(|| DatasetError::MissingVariable("visc".into()))?;
    ...
    pub fn get_initial_condition(&self, idx: usize) -> Array1<f64> {
        self.input.slice(ndarray::s![idx, ..]).to_owned()
    }

    pub fn get_solution(&self, idx: usize) -> Array2<f64> {
        self.output.slice(ndarray::s![idx, .., ..]).to_owned()
    }
}
...
mod tests {
    ...
    fn burgers_from_parts_matches_julia_shapes() {
        ...
        assert_eq!(ds.get_solution(0), array![[1.0, 2.0, 3.0], [1.1, 2.1, 3.1]]);
    }

    #[test]
    fn darcy_accessors_match_expected() {
        let sol = array![[[1.0, 2.0], [3.0, 4.0]], [[0.1, 0.2], [0.3, 0.4]],];
        let coeff = array![[[10.0, 20.0], [30.0, 40.0]], [[1.0, 2.0], [3.0, 4.0]],];
